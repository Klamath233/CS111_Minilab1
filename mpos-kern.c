...
static pid_t create_thread(void (*start_function)(void));
...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_EXIT:
		...
		if (current->p_waiter) {
			proc_array[current->p_waiter].p_state = P_RUNNABLE;
			proc_array[current->p_waiter].p_registers.reg_eax = current->p_exit_status;
		}
		current->p_waiter = 0;
	...
	case INT_SYS_WAIT: {
		...
		if (p <= 0 || p >= NPROCS || p == current->p_pid
			...
			// Add the waiting process to the waiting queue of the
			// waited process.
			if (!proc_array[p].p_waiter) {
				proc_array[p].p_waiter = current->p_pid;
				current->p_state = P_BLOCKED; // Set current process to be blocked.
	...
	case INT_SYS_NEW_THREAD: {
		app_printf("in case statements\n");
		current->p_registers.reg_eax = create_thread((void (*)(void))
...
/* This cannot work yet.
static pid_t
create_thread(void (*start_function)(void))
{
	pid_t pid;
	app_printf("creating thread\n");
	for (pid = 1; pid < NPROCS; pid++) {
		if (proc_array[pid].p_state == P_EMPTY) {
			proc_array[pid].p_state = P_RUNNABLE;
			pecial_registers_init(&proc_array[pid]);
			proc_array[pid].p_registers.reg_eip = (uint32_t) start_function;
			proc_array[pid].p_registers.reg_esp = PROC1_STACK_ADDR
					+ pid * PROC_STACK_SIZE;
			return pid;
		}
	}
...
copy_stack(process_t *dest, process_t *src)
{
	...
	src_stack_top = PROC1_STACK_ADDR
					+ src->p_pid * PROC_STACK_SIZE/* YOUR CODE HERE */;
	src_stack_bottom = src->p_registers.reg_esp;
	uint32_t stack_size = src_stack_top - src_stack_bottom;
	dest_stack_top = PROC1_STACK_ADDR
					 + dest->p_pid * PROC_STACK_SIZE/* YOUR CODE HERE */;
	dest_stack_bottom = dest_stack_top - stack_size;/* YOUR CODE HERE: calculate based on the other variables */;
	memcpy((void *)dest_stack_bottom, (const void *)src_stack_bottom,
			stack_size);
